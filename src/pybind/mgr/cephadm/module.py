    ...
    def remove_service(self, service_name, force=False):
        ...
        for host, dm in self.cache.daemons.items():
            for name, d in dm.items():
                if d.matches_service(service_name):
                    args.append(
                        ('%s.%s' % (d.daemon_type, d.daemon_id), d.nodename, force)
                    )
        if not args:
...
""".format(
    ...
    def remove_osds(self, osd_ids: List[str],
        ...
        daemons = self.cache.get_daemons_by_type('osd')
        found = set()
    ...
    def _remove_osds_bg(self) -> Optional[bool]:
        """
    ...
    def get_pg_count(self, osd_id: str) -> int:
        ...
        self.log.debug("Querying for drain status")
        ret, out, err = self.mon_command({
